  ...
  $.ajax({
    ...
    success: function (response) {
      if (response != null) {
        $("#mainbinder").empty();
        $("#mainbinder").append(
          '<label><b>First Name*</label><br><input type="text" name="firstName" id="firstName" placeholder="First Name " value="' +
            response.firstName +
            '"><br><br><label><b>Last Name*</label><br>            <input type="text" name="lastName" id="lastName" placeholder="Last Name" value="' +
            response.lastName +
            '">            <br><br>            <label><b>Date of Birth*                           </label><br>            <input type="text" name="dob" id="dob" placeholder="MM/DD/YYYY" value="' +
            response.dob +
            '">            <br><br>            <label><b>E-mail*                           </label> <br>           <input type="email" name="email" id="email" placeholder="E-mail" value="' +
            response.email +
            '" disabled>            <br><br>            <label><b>Gender*                           </label> <br>           <select name="gender" id="gender">                <option value="m">Male</option>                <option value="f">Female</option>                <option value="o">other</option>            </select>            <br><br>            <label><b>Phone Number*                           </label> <br>           <input type="text" name="phone_number" id="phone_number" placeholder="Phone Number"                value="' +
            response.phone_number +
            '">            <br><br>            <label><b>Address*                           </label> <br>           <input type="text" name="address" id="address" placeholder="Address" value="' +
            response.address +
            '">            <br><br>            <label><b>Zipcode*                           </label> <br>           <input type="text" name="zipcode" id="zipcode" placeholder="Zipcode" value="' +
            response.zipcode +
            '">            <br><br><button class="btn btn-primary" id="update_user" onclick="updateowner(\'' +
            response.email +
            "')\">Update</button>"
        );
      } else {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response != null) {
        $("#mainbinder").empty();
        $("#mainbinder").append(
          '<label><b>Dog name*                           </label><br>            <input type="text" name="dog_name" id="dog_name" placeholder="Dog Name" value="' +
            response.dog_name +
            '">            <br><br>            <label><b>Dog Gender*                           </label> <br>               <select name="dog_gender" id="dog_gender">                <option value="m">Male</option>                <option value="f">Female</option>            </select>            <br><br>            <label><b>Dog Breed*                           </label>   <br>         <input type="text" name="dog_breed" id="dog_breed" placeholder="Dog Breed" value="' +
            response.dog_breed +
            '">            <br><br>            <label><b>Dog Date of Birth*                           </label>    <br>        <input type="text" name="dog_dob" id="dog_dob" placeholder="MM/DD/YYYY" value="' +
            response.dog_dob +
            '">            <br><br>            <label><b>Dog Weight*                           </label>     <br>       <input type="number" name="weight" id="weight" placeholder="Dog Weight" value="' +
            response.weight +
            '">            <br><br>            <label><b>Dog Behavioral Information                           </label>    <br>        <textarea type="text" name="behavioral_information" id="behavioral_information"                placeholder="Dog Behavioral Information"></textarea>            <br><br>            <label><b>Veterinarian name                           </label>  <br>          <input type="text" name="vet_name" id="vet_name" placeholder="veterinarian name" value="' +
            response.vet_name +
            '">            <br><br>            <label><b>Veterinarian Phone Number                           </label>   <br>         <input type="text" name="vet_phn" id="vet_phn" placeholder="veterinarian name" value="' +
            response.vet_phn +
            '">            <br><br><button class="btn btn-primary" id="update_Dog" onClick="updatdog(\'' +
            response.email +
            "')\">Update</button>"
        );
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.length != 0) {
        for (var i = 0; i < response.length; i++) {
          $("#mainbinder").append(
            '<div class="card text-center"><div class="card-header"></div><div class="card-body"><h5 class="card-title">' +
              response[i].firstName +
              " " +
              response[i].lastName +
              '</h5><p class="card-text">' +
              ...
              ' </label><br><label for="price">Price:' +
              response[i].price +
              '$</label><br><a href="#" class="btn btn-primary">Book</a></div><div class="card-footer text-muted"></div></div>'
          );
        }
      } else {
